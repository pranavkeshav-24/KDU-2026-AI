...
from utils.langchain_utils import (
    ...
    analyze_candidate_profiles,
    generate_interview_questions,
    generate_hiring_scorecard,
)
...
class AnalyzeProfiles(BaseModel):
    file_path: Annotated[str, Field(description="Path to the resume PDF file")]
    job_description: Annotated[str, Field(description="Job description to rate candidate profiles against")]

class GenerateInterviewQuestions(BaseModel):
    file_path: Annotated[str, Field(description="Path to the resume PDF file")]
    job_description: Annotated[str, Field(description="Job description to generate questions for")]


class GenerateHiringScorecard(BaseModel):
    file_path: Annotated[str, Field(description="Path to the resume PDF file")]
    job_description: Annotated[str, Field(description="Job description to generate a hiring scorecard for")]
...
async def list_tools():
    return [
        ...
        Tool(
            ...
            description="Rate and analyze LinkedIn and GitHub profiles found in the candidate's resume against a job description.",
            inputSchema=AnalyzeProfiles.model_json_schema(),
        ),
        Tool(
            name="generate_interview_questions",
            description="Generate tailored Technical and Behavioral interview questions based on the candidate's resume and job description.",
            inputSchema=GenerateInterviewQuestions.model_json_schema(),
        ),
        Tool(
            name="generate_hiring_scorecard",
            description="Create an evidence-based hiring scorecard with a final recommendation and a 30-60-90 day ramp plan.",
            inputSchema=GenerateHiringScorecard.model_json_schema(),
        ),
    ]
...
async def call_tool(name, arguments):
    ...
    if name == "match_resume":
        ...
        # Step 3: Find relevant sections (embedding search with lexical fallback)
    ...
    elif name == "analyze_profiles":
        try:
            args = AnalyzeProfiles(**arguments)
        except ValueError as e:
            raise McpError(INVALID_PARAMS, str(e))
        ...
        analysis = analyze_candidate_profiles(resume_text, job_description, llm)
        
        response = f"Profile Analysis for '{file_path}':\n\n"
        response += analysis
        ...
        questions = generate_interview_questions(resume_text, job_description, llm)
        
        response = f"Tailored Interview Questions for '{file_path}':\n\n"
        response += questions
            
        return [TextContent(type="text", text=response)]

    elif name == "generate_hiring_scorecard":
        try:
            args = GenerateHiringScorecard(**arguments)
        except ValueError as e:
            raise McpError(INVALID_PARAMS, str(e))
        ...
        scorecard = generate_hiring_scorecard(resume_text, job_description, llm)

        response = f"Hiring Scorecard for '{file_path}':\n\n"
        response += scorecard
