...
| Primary Model | `google/gemma-3-27b-it:free` — general reasoning & multimodal |
| Secondary Model | `meta-llama/llama-4-scout:free` — fast lightweight responses |
| Tertiary Model | `mistralai/mistral-small-3.2-24b-instruct:free` — structured output |
...
│   google/gemma-3-27b-it:free       (default / vision)        │
│   meta-llama/llama-4-scout:free    (fast / lightweight)       │
│   mistralai/mistral-small-3.2-24b-instruct:free  (JSON out)  │
└───────────────────────────┬──────────────────────────────────┘
                            │
┌───────────────────────────▼──────────────────────────────────┐
...
# models/router.py
TASK_MODEL_MAP = {
    'vision':     'google/gemma-3-27b-it:free',                      # multimodal
    'reasoning':  'google/gemma-3-27b-it:free',                      # general reasoning
    'fast':       'meta-llama/llama-4-scout:free',                    # simple / fast queries
    'structured': 'mistralai/mistral-small-3.2-24b-instruct:free',   # JSON output
}
...
def get_llm(task: str) -> ChatOpenAI:
    model_id = TASK_MODEL_MAP.get(task, 'google/gemma-3-27b-it:free')
    return ChatOpenAI(
...
|---|---|---|
| `google/gemma-3-27b-it:free` | Vision, reasoning, default fallback | Multimodal, strong context handling |
| `meta-llama/llama-4-scout:free` | Fast single-turn responses | Low latency; 10M context window |
| `mistralai/mistral-small-3.2-24b-instruct:free` | Structured JSON output | Instruction-tuned; follows schemas reliably |
...
*End of Document — Multimodal AI Assistant Design Doc v1.0*
